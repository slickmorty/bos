...
class ResidualBlock(Layer):
    ...
    def build(self, input_shape):
        ...
        with K.name_scope(self.name):
            ...
            for k in range(2):  # dilated conv block.
                ...
                with K.name_scope('act_and_dropout_{}'.format(k)):
                    self._build_layer(Activation(
                        self.activation, name='Act_Conv1D_{}'.format(k)))
                    self._build_layer(SpatialDropout1D(
                        rate=self.dropout_rate, name='SDropout_{}'.format(k)))
            ...
            self._build_layer(Activation(
                self.activation, name='Act_Conv_Blocks'))
            self.final_activation = Activation(
                self.activation, name='Act_Res_Block')
...
class TCN(Layer):
    ...
    def __init__(self,
        ...
        # edge case if padding='same' and time_dim = None
    ...
    def compute_output_shape(self, input_shape):
        ...
        if not self.return_sequences:
            ...
            batch_size = batch_size.value if hasattr(
                batch_size, 'value') else batch_size
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        for res_block in self.residual_blocks:
            try:
                ...
                x, skip_out = res_block(
                    K.cast(x, 'float32'), training=training)
